...
var complexCases = map[string]complexTest{
	"flip": {
		"(lambda (x y) (list y x))",
		[]token{
			token{tokenLeftParen, 1, 1, "("},
			token{tokenAtom, 7, 1, "lambda"},
			token{tokenLeftParen, 9, 1, "("},
			token{tokenAtom, 10, 1, "x"},
			token{tokenAtom, 12, 1, "y"},
			token{tokenRightParen, 13, 1, ")"},
			token{tokenLeftParen, 15, 1, "("},
			token{tokenAtom, 19, 1, "list"},
			token{tokenAtom, 21, 1, "y"},
			token{tokenAtom, 23, 1, "x"},
			token{tokenRightParen, 24, 1, ")"},
			token{tokenRightParen, 25, 1, ")"},
		},
	},
	"multiline": {
