...
func NewVM() *VM {
	globals := LoadBuiltins()
	return &VM{stack: newStack(), globals: globals, macros: make(map[runtime.Atom]runtime.Macro)}
}

func (vm *VM) child() *VM {
	return &VM{stack: newStack(), globals: vm.globals, macros: vm.macros}
}
