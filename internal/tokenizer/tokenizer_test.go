...
// Benchmark tests
func BenchmarkTokenizeSimpleList(b *testing.B) {
	source := "(+ 1 2 3)"
	for b.Loop() {
		tokenizer := NewTokenizer(source)
		for {
...
func BenchmarkTokenizeLargeAtom(b *testing.B) {
	source := strings.Repeat("a", 1000)
	for b.Loop() {
		tokenizer := NewTokenizer(source)
		tokenizer.NextToken()
...
func BenchmarkTokenizeNumbers(b *testing.B) {
	source := "123 456.789 0xFF 0o755 0b101 -999 +888"
	for b.Loop() {
		tokenizer := NewTokenizer(source)
		for {
			token := tokenizer.NextToken()
...
func BenchmarkTokenizeWithComments(b *testing.B) {
	source := `; This is a comment
(define factorial ; another comment
  (lambda (n) ; yet another
    (if (= n 0)
        1
        (* n (factorial (- n 1)))))) ; final comment`
	for b.Loop() {
		tokenizer := NewTokenizer(source)
		for {
			token := tokenizer.NextToken()
